Feature: Issuing one token
  ...
  Scenario: Customer with at least one token tries to get one token issued
     ...
     When the event CustomerTokensRequest "CreateTokensRequested" is sent
     Then a response CustomerTokensReturned "ResponseTokensCreated" is sent and a customer receives a list of tokens
  ...
  Scenario: Customer with no tokens tries to get one token issued
     ...
     When the event CustomerTokensRequest "CreateTokensRequested" is sent
     Then a response CustomerTokensReturned "ResponseTokensCreated" is sent and the system throws an exception with message "You have no more tokens. Request more tokens."
